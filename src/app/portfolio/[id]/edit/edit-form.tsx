...
export default function EditPortfolioForm({ id, initialValues }: Props) {
    ...
    const onSubmit: SubmitHandler<FormValues> = (data) => {
        ...
        service.updatePortfolio(id, data).then(() => {
            router.push(`/portfolio/${id}`);
        }).catch((error: Error) => {
            setErrorMessage(error.message || "Failed to update portfolio");
        });
    };
            ...
            <main className="w-full max-w-2xl">
                <div className="mb-8 text-center sm:text-left">
                    ...
                    <p className="mt-3 text-base leading-7 text-gray-300">
                        Actualiza la informacion principal que se muestra en tu portfolio.
                    </p>
                </div>
                ...
                <div className="flex flex-col items-center w-full gap-6 text-center sm:items-start sm:text-left">
                    <Card className="w-full border-white/20 bg-white/10 text-white shadow-2xl backdrop-blur-md">
                        <CardHeader>
                            <CardTitle className="text-2xl font-bold">Datos del portfolio</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
                                ...
                                <div className="flex flex-col gap-3 pt-3 sm:flex-row">
                                    ...
                                    <Link href={`/portfolio/${id}`} className="flex-1">
                                        <Button
                                            type="button"
                                            variant="outline"
